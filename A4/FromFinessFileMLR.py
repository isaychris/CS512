...
class FromFinessFileMR:
    ...
    def validate_model(self, model, fileW, population, TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
        ...
        for i in range(numOfPop):
            ...
            q2_loo = self.fff.getTwoDecPoint(q2_loo)

            r2pred_validation = self.r2Pred(TrainY, ValidateY, Yhat_validation)
            r2pred_validation = self.fff.getTwoDecPoint(r2pred_validation)

            r2pred_test = self.r2Pred(TrainY, TestY, Yhat_test)
            r2pred_test = self.fff.getTwoDecPoint(r2pred_test)
            ...
            trackFitness[idx] = self.fff.getTwoDecPoint(fitness[i])

            trackModel[idx] = model_desc

            trackR2[idx] = self.fff.getTwoDecPoint(r2_train)
            trackQ2[idx] = self.fff.getTwoDecPoint(q2_loo)
            trackR2PredValidation[idx] = self.fff.getTwoDecPoint(r2pred_validation)
            trackR2PredTest[idx] = self.fff.getTwoDecPoint(r2pred_test)

            yTrain[idx] = TrainY.tolist()

            yHatTrain[idx] = Yhat_train.tolist()
            for i in range(len(yHatTrain[idx])):
                yHatTrain[idx][i] = self.fff.getTwoDecPoint(yHatTrain[idx][i])

            yHatCV[idx] = Yhat_cv.tolist()
            for i in range(len(yHatCV[idx])):
                yHatCV[idx][i] = self.fff.getTwoDecPoint(yHatCV[idx][i])

            yValidation[idx] = ValidateY.tolist()

            yHatValidation[idx] = Yhat_validation.tolist()
            for i in range(len(yHatValidation[idx])):
                yHatValidation[idx][i] = self.fff.getTwoDecPoint(yHatValidation[idx][i])

            yTest[idx] = TestY.tolist()

            yHatTest[idx] = Yhat_test.tolist()
            for i in range(len(yHatTest[idx])):
                yHatTest[idx][i] = self.fff.getTwoDecPoint(yHatTest[idx][i])

        self.write(fileW, trackDesc, trackFitness, trackModel, trackR2, \
              trackQ2, trackR2PredValidation, trackR2PredTest)
        return itFits, fitness


    def write(self, fileW, trackDesc, trackFitness, trackModel, trackR2, \
              trackQ2, trackR2PredValidation, trackR2PredTest):
